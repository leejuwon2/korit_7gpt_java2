...
/*
    ...
    5. DIP(의존 역전 원칙)
        - 고수준 모듈이 저수준 모듈에 의존하지 않고, 추상화에 의존해야한다.
 */

// === SOLID 원칙 예시 (주문 관리 시스템) === //

// 1. 단일 책임 원칙(SRP): 각 클래스는 하나의 책임만을 가짐
class Order {
...
class CreditCardPaymentProcessor implements PaymentProcessor{
    ...
    public void processPayment(Order order) {
        System.out.println("신용카드로 결제를 진행햡니다: " + order.getOrderId());
    }
}
...
// ISP: 클라이언트가 불필요한 메서드에 의존하지 않도록
interface GenerateReport {
    void generateReport(Order order);
}

class PdfReportGnerator implements GenerateReport {
    @Override
...
public class C_OOP {
    public static void main(String[] args) {
        // CreditCardPaymentProccessor 를 결제 처리기로 선택
        PaymentProcessor paymentProcessor = new CreditCardPaymentProcessor();

//        PdfReportGnerator를 보고서 생성기로 선택
        GenerateReport generateReport = new PdfReportGnerator();
