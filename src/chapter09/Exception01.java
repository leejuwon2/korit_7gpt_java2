...
// >> 예외와 관련된 메시지, 처리 방법에 대한 예시 정보를 포함하고 있음
...
// 1-1) Checked Exception (코드에 빨간줄)
// : 컴파일 시점에 체크되는 예외
// - 개발자가 직접 반드시 처리
// - 컴파일러는 해당 예외가 처리되어 있지 않을 경우 오류를 발생


// 1-2) Runtime(Unchecked) Exception (코드에는 오류가 안보임)
// : 런타임 시점에 발생되는 예외
// - 개발자의 실수로 발생하는 예외
...
// - 컴파일: 소스 코드를 기계어로 번역하는 단계, 실행되지 않고 코드를 분석하고 변환하는 과정에만 국한
    // 자바에서는 .java 파일이 컴파일되어 .class 파일(바이트코드)로 생성 (src 위에 out 패키지)
...
/*
    ...
    try 뒤에 예외가 발생할 수 있는 코드에서 발생할 수 있는 예외들을
    catch 뒤에 써 줘야함
 */
    ...
    public static void main(String[] args) {
        ...
        // System.out.println(c1); - 컴파일 예외
         ...
         // Exception in thread "main" java.lang.ArithmeticException: / by zero
          //at chapter09.Exception01.main(Exception01.java:55)
        try{
            // 예외 발생 가능성 코드 작성
            int division = 10/0;
            // : 정수를 0으로 나눌 수 없다.
        } catch (Exception e) {
            // try 블럭 내에서 발생하는 예외를 () 소괄호 내에서 매개변수로 전달받아 {} 실행문에서 처리
        ...
        // : 인자로 전달되는 문자열 예외 메시지는 getMessage() 메서드를 통해 출력 가능
