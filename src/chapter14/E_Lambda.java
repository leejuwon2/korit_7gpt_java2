...
public class E_Lambda {
    public static void main(String[] args) {
        ...
        Consumer<String> printLegnth = msg -> System.out.println(msg.length());

        printMessage.accept("안녕하세요 :)");

        Consumer<String> combinedConsumer = printMessage.andThen(printLegnth);
        combinedConsumer.accept("안녕하세요 :)"); // 8

        // === Supplier === //
        Supplier<Double> randomValue = () -> Math.random(); // 0.0과 0.1 사이의 인수
        System.out.println(randomValue.get()); //0.39128639791257713
    }
}
