...
/*
    ...
    - 필요한 값이 개수만큼 변수를 선언, 데이터 타입과 변수명을 쉼표(,)로 구분하여 나열
...
*/
class Example {
    int x, y; // (인스턴스) 변수 - 데이터 타입이 일치 할 경우 생략가능

    int add(int x, int y) { // 더하다. x, y 는 매개변수
        int result = x + y;
...
public class Method02 {
    public static void main(String[] args) {
        ...
        // 함수의 파라미터에 데이터 전달
        // : 함수 호출 시 () 영역 내에서 parameter에 전달되는 값을 argument 라고 부름

        // 매개변수(Parameter) VS 인자(인수, Argument)

        // 1. 매개변수
        // - 메서드를 정의할 때 괄호 안에 선언되는 변수
        // - 메서드가 호출될 때 값을 전닯다기 위한 입력변수로 사용
        // > 메서드의 선언부에 정의, 호출 시 전달받은 값은 내부적으로 사용

        // 2. 인자(인수)
        // - 메서드를 호출 할 때 괄호 안에 전달하는 '값'
        ...
        // examle.add("10" ,"20"); - Error (인자값은 매개변수의 타입과 같거나 자동 형 변환이 가능해야한다)
        example.add('a','b'); // char 타입(2) >> int 타입 (4) : 자동 형 변환


        System.out.println(example.add(10,20)); // 10,20 은 인자
    }
}
